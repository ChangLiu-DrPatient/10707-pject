...
if __name__ == '__main__':
    ...
    # assert 1==0
    X_train = np.load('xtrain_na_{}.npy'.format(args.task))
    y_train = np.load('ytrain_na_{}.npy'.format(args.task))
    X_val = np.load('xval_na_{}.npy'.format(args.task))
    y_val = np.load('yval_na_{}.npy'.format(args.task))
    ...
    for epoch in track(range(epochs), description='fine tuning network'):
        ...
        if accuracy_val > best_acc:
            ...
            torch.save(model.state_dict(), 'models/finetune_na_{}_{}_{}.pt'.format(args.task, args.baseline, args.free))
    with open('results/trajectories_finetune_na_{}_{}_{}.pkl'.format(args.task, args.baseline, args.free), 'wb') as f:
