...
def get_args():
    ...
    parser.add_argument("--data-split", default=0.345, type=float)
    ...
    parser.add_argument("--usena", default='', type=str, choices=['', '_usena'], help='whether to use N/As')
...
if __name__ == '__main__':
    ...
    with open('../multi_species_data/multi_species_{}_na.txt'.format(args.task), 'r') as f:
        label_strs_na =  [(x.split('\t')[0], x.split('\t')[1].strip()) for x in f.readlines()]
    with open('../multi_species_data/multi_species_{}_nna.txt'.format(args.task), 'r') as f:
        label_strs_nna =  [(x.split('\t')[0], x.split('\t')[1].strip()) for x in f.readlines()]
    data = np.load('../multi_species_data/multi_species_data_{}_nna.npy'.format(args.task), allow_pickle=True)
    data_na = np.load('../multi_species_data/multi_species_data_{}_na.npy'.format(args.task), allow_pickle=True)

    labels_na = np.array([label_dict[x] for x in label_strs_na])
    labels_na = np.array([-1 for x in range(len(labels_na))])
    labels = np.array([label_dict[x] for x in label_strs_nna])
    ...
    if pad_len > 0:
        ...
        pad_data_na = np.zeros((data_na.shape[0], pad_len))
        data_na = np.concatenate((data_na, pad_data_na), axis=1)
    data = data[:, np.newaxis, :]
    data_na = data_na[:, np.newaxis, :]

    print("fine tune data shape: nna {}; na {}".format(data.shape, data_na.shape))
    ...
    if args.usena == '_usena':
        X_train = np.concatenate((X_train, data_na), axis=0)   # incorporate nonzeros
        y_train = np.concatenate((y_train, labels_na))
    np.save('xtrain_na_{}{}.npy'.format(args.task, args.usena), X_train)
    np.save('ytrain_na_{}.npy'.format(args.task), y_train)
    np.save('xval_na_{}.npy'.format(args.task), X_val)
    np.save('yval_na_{}.npy'.format(args.task), y_val)
    ...
    for epoch in track(range(epochs), description='fine tuning network'):
        ...
        if accuracy_val > best_acc:
            ...
            torch.save(model.state_dict(), 'models/finetune_na_{}_{}_{}{}.pt'.format(args.task, args.coeff, args.free, args.usena))
    with open('results/trajectories_finetune_na_{}_{}_{}{}.pkl'.format(args.task, args.coeff, args.free, args.usena), 'wb') as f:
